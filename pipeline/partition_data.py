...
def partition_data_and_save_to_disk(
    X_data, y_data, X_save_name, y_save_name, save_size, random_state
):
    (
        X_partition_to_save,
        X_remaining,
...
def main():
    ...
    X_not_train, y_not_train = partition_data_and_save_to_disk(
        X, y, "data/X_train.csv", "data/y_train.csv", 0.6, 1
    )
    X_holdout_and_blind, y_holdout_and_blind = partition_data_and_save_to_disk(
        X_not_train, y_not_train, "data/X_test.csv", "data/y_test.csv", 0.5, 2
    )
    X_blind, y_blind = partition_data_and_save_to_disk(
        X_holdout_and_blind,
        y_holdout_and_blind,
        "data/X_holdout.csv",
        "data/y_holdout.csv",
